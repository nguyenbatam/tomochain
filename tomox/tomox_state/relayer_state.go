...
func CheckSubTokenBalance(addr common.Address, value *big.Int, token common.Address, statedb *state.StateDB) (*big.Int, error) {
	// TOMO native
	if token.String() == common.TomoNativeAddress {
		balance := statedb.GetBalance(addr)
		if balance.Cmp(value) < 0 {
...
func CheckAddTokenBalance(addr common.Address, value *big.Int, token common.Address, statedb *state.StateDB) (*big.Int, error) {
	// TOMO native
	if token.String() == common.TomoNativeAddress {
		balance := statedb.GetBalance(addr)

...
func (tomox *TomoX) getTradeQuantity(coinbase common.Address, ipcEndpoint string, statedb *state.StateDB, takerOrder *tomox_state.OrderItem, makerOrder *tomox_state.OrderItem, quantityToTrade *big.Int) (*big.Int, bool, error) {
	...
	if quantity.Sign() > 0 {
		// Apply Match Order
		setteBalance, err := GetSettleBalance(takerOrder.Side, takerFeeRate, makerOrder.BaseToken, makerOrder.QuoteToken, makerOrder.Price, makerFeeRate, baseTokenDecimal, quantityToTrade)
		if err != nil {
...
func SetteBalance(coinbase common.Address, takerOrder, makerOrder *tomox_state.OrderItem, settleBalance *SettleBalance, statedb *state.StateDB) error {
	...
	//Checking balance
	newTakerInTotal, err := tomox_state.CheckAddTokenBalance(takerOrder.UserAddress, settleBalance.taker.InTotal, settleBalance.taker.InToken, statedb)
	if err != nil {
		return err
	}
	newTakerOutTotal, err := tomox_state.CheckSubTokenBalance(takerOrder.UserAddress, settleBalance.taker.OutTotal, settleBalance.taker.OutToken, statedb)
	if err != nil {
		return err
	}

	newMakerInTotal, err := tomox_state.CheckAddTokenBalance(makerOrder.UserAddress, settleBalance.maker.InTotal, settleBalance.maker.InToken, statedb)
	if err != nil {
		return err
	}
	newMakerOutTotal, err := tomox_state.CheckSubTokenBalance(makerOrder.UserAddress, settleBalance.maker.OutTotal, settleBalance.maker.OutToken, statedb)
	if err != nil {
		return err
	}

	newTakerFee, err := tomox_state.CheckAddTokenBalance(takerExOwner, settleBalance.taker.Fee, makerOrder.QuoteToken, statedb)
	if err != nil {
		return err
	}

	newMakerFee, err := tomox_state.CheckAddTokenBalance(makerExOwner, settleBalance.maker.Fee, makerOrder.QuoteToken, statedb)
	if err != nil {
		return err
	}

...
type DumpExchange struct {
	...
	OrderStorage map[string]string `json:"orderStorage"`
}
...
func (self *StateDB) RawDump() Dump {
	...
	for it.Next() {
		...
		account := DumpExchange{
			...
			OrderRoot:    common.Bytes2Hex(data.OrderRoot[:]),
			AskStorage:   make(map[string]string),
			BidStorage:   make(map[string]string),
			OrderStorage: make(map[string]string),
		}
		...
		storageIt = trie.NewIterator(obj.getAsksTrie(self.db).NodeIterator(nil))
		for storageIt.Next() {
			account.OrderStorage[common.Bytes2Hex(self.trie.GetKey(storageIt.Key))] = common.Bytes2Hex(storageIt.Value)
		}

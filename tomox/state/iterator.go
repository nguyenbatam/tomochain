...
type NodeIterator struct {
	...
	Hash   common.Hash // Price of the current entry being iterated (nil if not standalone)
	Parent common.Hash // Price of the first full ancestor node (nil if current is the root)
...
func (it *NodeIterator) step() error {
	...
	dataTrie, err := it.state.db.OpenStorageTrie(common.BytesToHash(it.stateIt.LeafKey()), account.AskRoot)
	if err != nil {
		return err
	}

...
func (t *Trie) TryGetBestRight() ([]byte, error) {
	value, newroot, didResolve, err := t.tryGetBestRight(t.root, []byte{})
	if err == nil && didResolve {
		t.root = newroot
	}
	return hexToKeybytes(value), err
}

func (t *Trie) tryGetBestRight(origNode Node, key []byte) (value []byte, newnode Node, didResolve bool, err error) {
	switch n := (origNode).(type) {
	case nil:
		return nil, nil, false, nil
	case *ShortNode:
		...
		value, newnode, didResolve, err = t.tryGetBestRight(n.Val, append(key, n.Key...))
		if err == nil && didResolve {
			n = n.copy()
			n.Val = newnode
			n.flags.gen = t.cachegen
	...
	case *FullNode:
		for i := byte(len(n.Children) - 1); i >= 0; i-- {
			...
			value, newnode, didResolve, err = t.tryGetBestRight(n.Children[i], append(key, i))
			if err == nil && didResolve {
				n = n.copy()
				n.flags.gen = t.cachegen
				n.Children[i] = newnode
	...
	case HashNode:
		...
		value, newnode, _, err := t.tryGetBestRight(child, key)
		return value, newnode, true, err
	default:
		return nil, nil, false, fmt.Errorf("%T: invalid node: %v", origNode, origNode)
	}

...
var (
	...
	rttMinEstimate   = 1 * time.Second          // Minimum round-trip time to target for download requests
	rttMaxEstimate   = 5 * time.Second          // Maximum rount-trip time to target for download requests
	rttMinConfidence = 0.1                      // Worse confidence factor in our estimated RTT value
	ttlScaling       = 1                        // Constant scaling factor for RTT -> TTL conversion
	ttlLimit         = 3 * time.Second          // Maximum TTL allowance to prevent reaching crazy timeouts
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td *big.Int) (err error) {
	log.Debug("Start wait to begin download")
...
func (d *Downloader) spawnSync(fetchers []func() error) error {
	...
	for i := 0; i < len(fetchers); i++ {
		if i == len(fetchers)-1 {
			...
			log.Debug("Notify close queue synchronise content")
			d.queue.Close()
		}
		if err = <-errc; err != nil {
			break
...
func (d *Downloader) fetchHeaders(p *peerConnection, from uint64, pivot uint64) error {
	...
	getHeaders := func(from uint64) {
		...
		ttl = d.requestTTL()
		p.log.Debug("TTL header downloads", "ttl", ttl)
		timeout.Reset(ttl)

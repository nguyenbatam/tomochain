...
type TransactionReader interface {
	...
	// staked yet. Note that the transaction may not be part of the canonical chain even if
	// it's not pending.
	TransactionByHash(ctx context.Context, txHash common.Hash) (tx *types.Transaction, isPending bool, err error)
	// TransactionReceipt returns the receipt of a staked transaction. Note that the
...
// the EVM but not staked into the blockchain. ContractCall is a low-level method to
...
// true gas limit requirement as other transactions may be added or removed by stakers, but

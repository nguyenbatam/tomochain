...
func (bc *BlockChain) PrepareBlock(block *types.Block) (err error) {
	...
	if err == nil {
		...
		log.Debug("Stop prepare a block becasue inserting", "number", block.NumberU64(), "hash", block.Hash(), "validator", block.Header().Validator)
		return nil
	}
	return err
}
...
func (bc *BlockChain) getResultBlock(block *types.Block, verifiedM2 bool) (*ResultProcessBlock, error) {
	...
	if verifiedM2 {
		if result, check := bc.resultProcess.Get(block.Header().HashNoValidator()); check {
...
func (bc *BlockChain) insertBlock(block *types.Block) ([]interface{}, []*types.Log, error) {
	...
	if _, check := bc.downloadingBlock.Get(block.Hash()); check {
		log.Debug("Stop fetche a block becasue downloading", "number", block.NumberU64(), "hash", block.Hash())
	...
	defer bc.resultProcess.Remove(block.Header().HashNoValidator())
	bc.wg.Add(1)
	defer bc.wg.Done()
	// Write the block to the chain and get the status.
	bc.chainmu.Lock()
...
func (bc *BlockChain) PostChainEvents(events []interface{}, logs []*types.Log) {
	defer log.Debug("Finish PostChainEvents ", "events", len(events))

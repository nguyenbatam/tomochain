...
func (self *worker) commitNewWork() {
	...
	if parent.NumberU64() < self.lastBlockCommit {
		log.Debug("Reject becasue It's just commited block ", "number", self.lastBlockCommit)
	}
	...
	if atomic.LoadInt32(&self.mining) == 1 {
		...
		self.lastBlockCommit = work.Block.Number().Uint64()

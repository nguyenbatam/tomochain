...
func main() {
	toDB, err = ethdb.NewLDBDatabase("/data/tomo/copy", eth.DefaultConfig.DatabaseCache, utils.MakeDatabaseHandles())
	defer toDB.Close()
	//core.WriteCanonicalHash(toDB,common.HexToHash("0x9326145f8a2c8c00bbe13afc7d7f3d9c868b5ef39d89f2f4e9390e9720298624"),0)
	genesiHash := core.GetCanonicalHash(toDB, 0)
	genesisBlock := core.GetBlock(toDB, genesiHash, 0)
	genesisTd := core.GetTd(toDB, genesiHash, 0)
	fmt.Println("genesiHash",genesiHash)
	fmt.Println("genesisBlock",genesisBlock)
	fmt.Println("genesisTd",genesisTd)
	//flag.Parse()
